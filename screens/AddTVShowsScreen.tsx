    ...
    <View style={{height: "100%"}}>
      <SearchBar
        ...
        onChangeText={(text)=>{
          setState(text);
          if(text.length >= 3){
            searchTVShow(text).then(res => setResults(res));
          } else {
            setResults([]);
          }
        }}
...
function Post(props: {data: tvShow, scheme: "light" | "dark"}){
  ...
  getTVShowGenre(props.data.genre_ids[0]).then((res: string | undefined='') => setGenre(res));
  return(
    <View style={{
      borderRadius: 10,
      margin: '2%',
